...
Toggles.prototype.createEl = function() {
  ...
  //alert('height ' + height + ' widht ' + width);

  // if the element doesn't have an explicit height/width in css, set them
  if (!height) self.el.height(height = self.opts['height']);
  if (!width) self.el.width(width = self.opts['width']);

  self.h = height;
  self.w = width;
  //alert('height2 ' + self.h + ' widht2 ' + self.w);
  ...
  var onOffWidth = width / 2; //width - halfHeight;
  //alert('halfHeight ' + halfHeight + ' onOffWidth ' + onOffWidth);
...
Toggles.prototype.toggle = function(state) {
  ...
  // check we aren't already in the desired state
